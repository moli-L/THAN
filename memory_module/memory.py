...
class Memory(nn.Module):
  ...
  def __init_memory__(self):
    ...
    self.messages = defaultdict(list)  # {node_id: [(emb, ts), (emb, ts)]}
    self.msg_flag = np.zeros(self.n_nodes) # value is 1 if a node has message to update

  def store_raw_messages(self, nodes, node_id_to_messages):
    for node in nodes:
      self.messages[node].extend(node_id_to_messages[node])
    self.msg_flag[nodes] = 1
  ...
  def backup_memory(self):
    ...
    return self.memory.data.clone(), self.last_update.data.clone(), messages_clone, self.msg_flag.copy()

  def restore_memory(self, memory_backup):
    self.memory.data, self.last_update.data = memory_backup[0].clone(), memory_backup[1].clone()
    self.msg_flag = memory_backup[3].copy()
